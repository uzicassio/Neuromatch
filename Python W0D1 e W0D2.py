...
def plot_all(t_range, v, raster=None, spikes=None, spikes_mean=None):
  ...
  for j in range(n):
    plt.scatter(t_range, v[j], color="c", marker=".", alpha=0.01)
...
plt.plot(t_range, v_n.T, 'c', alpha=0.3)

plt.plot(t_range, v_n[-1], 'c', alpha=0.3, label='V(t)')
...
ax1 = plt.subplot(3, 1, 1)
for j in range(n):
  plt.scatter(t_range, v_n[j], color="c", marker=".", alpha=0.01)
plt.plot(t_range, v_mean, 'C1', alpha=0.8, linewidth=3)
plt.ylabel('$V_m$ (V)')
